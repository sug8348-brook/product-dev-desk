...
当前存储键为 `model-dev-board.projects.v1`。读取失败、版本不匹配或数据为空时，会回退到内置示例数据。
...
- 当前持久化依赖浏览器 `localStorage`，还不是正式数据库。
...
1. 拆分 `src/main.tsx`，为后续数据访问层和 Tauri 桌面化做准备。
2. 为导入/导出增加更友好的成功/失败提示。
